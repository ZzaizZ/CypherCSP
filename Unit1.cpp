...
void __fastcall TmainForm::sendButtonClick( TObject * Sender )
{
	...
	else
	{
		...
		else
		{
			...
			{
				clientThread =
					new ClientThread( ( sendOpenDialog->FileName + ".enc" )
					.c_str( ), ipEdit->Text.c_str( ), portEdit->Text.c_str( ),
					false );
				clientThread =
					new ClientThread( ( keyOpenDialog->FileName + ".encr" )
					.c_str( ), ipEdit->Text.c_str( ), portEdit->Text.c_str( ),
					false );
			}
		}
...
void __fastcall TmainForm::btnEncryptClick( TObject * Sender )
{
	...
	else
	{
		...
		crypt->DecryptFileW( ( odOpenEncFile->FileName ).c_str( ),
